...
/*! \file baseSpace.h"
 Degrees of freedom live in a space (e.g.: Euclidean space, a curved manifold, a
 cube with periodic boundary conditions, etc.) This base class promises an
 implementation of "displaceParticle", which takes a point and and a
 displacement vector, and updates the position of the point "distance", which
 takes a source point, a vector of target points, and fills vectors of distances
 and tangent vectors at the start and end of the paths.  Because of the eventual
 target of simulations on curved surfaces, we use the CGAL data types here.  For
 convenience, baseSpaces also promise an implementation of the ability to choose
 a random location (not necessarily uniformly sampled) and a random tangent
 vector to a point.
*/
